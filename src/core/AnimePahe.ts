...
export default class AnimePahe {
  ...
  static async getHome(ignoreCache = false) {
    return Prisma.cacheJSON("home", ignoreCache, CacheType.ANIMEPAHE_HOME, async () => {
  ...
  static async getAllAnime(ignoreCache = false) {
    const func = async () => {
    ...
    data.animes.forEach(async (element) => {
      await Prisma.client.animePaheAnimesID.upsert({
        where: {
          id: element.id,
        },
        update: {
  ...
  static async getAnime(session: string, ignoreCache = false) {
    return Prisma.cacheJSON(session, ignoreCache, CacheType.ANIMEPAHE_ANIME, async () => {
  ...
  static async getSource(animeSession: string, episodeSession: string, ignoreCache = false) {
    return Prisma.cacheJSON(`${animeSession}/${episodeSession}`, ignoreCache, CacheType.HIANIME_SOURCE, async () => {
